...
void eneymies_init(struct class_obj obj[], int x, int y, int dir, char *objname) {
    // obj = malloc(5 * sizeof(struct class_obj));
    for (int i = 0; i < 5; ++i) {
        obj[i].x = x;
        obj[i].y = y;
        obj[i].direction = dir;
        strcpy(obj[i].symbol, objname);
    }
}
...
void *display_enemies(void *arg) {
    ...
    while (1) {
        for (int i = 0; i < 5; ++i) {
            ...
            enemies[i].x--;
            enemies[i].y += rand;
            mvwprintw(win_en, enemies[i].y, enemies[i].x, enemies[i].symbol);
            wrefresh(win_en);
        }
        nanosleep(&ts, NULL);
        wclear(win_en);
...
void *display_bullet(void *arg) {
    ...
    for (int i = app_settings->X + 2; i < 35;) {
        i++;
        nanosleep(&ts, NULL);
        mvwprintw(app_settings->win, y, i, app_settings->ob.symbol);
